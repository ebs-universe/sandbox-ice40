.PHONY: help

help:
	@echo ""
	@echo "Usage:"
	@echo "  make <target> [VARIABLE=value]"
	@echo ""
	@echo "Build:"
	@echo "  build        Build FPGA bitstream (default)"
	@echo "  gui          Open nextpnr GUI"
	@echo ""
	@echo "Simulation:"
	@echo "  sim          Run all testbenches (full simulation build)"
	@echo "  sim-wave     List generated waveforms for all testbenches"
	@echo ""
	@echo "Test: (implemented for libraries only)"
	@echo "  test          Run all testbenches"
	@echo "  test-wave     Run all testbenches and generate waveforms"
	@echo ""
	@echo "Board / Hardware:"
	@echo "  flash        Program the target board"
	@echo ""
	@echo "Analysis & Tooling:"
	@echo "  pinout       Generate all pinout documentation formats"
	@echo "  rtlview      Generate RTL visualization (DOT/SVG)"
	@echo "  timing       Generate timing analysis reports"
	@echo "  analysis     Generate overall design analysis reports"
	@echo "  deps         Show library dependency graph (not functioning)"
	@echo ""
	@echo "Housekeeping:"
	@echo "  clean        Remove all build artifacts"
	@echo ""
	@echo "Variables:"
	@echo "  BOARD=<name>   Select target board"
	@echo "  DESIGN=<name>  Select design"
	@echo ""
